...
impl ByteStack {
    pub fn new2() -> Self {
        Self {
            stack: Vec::new(),
            // stack: ptr::null_mut(), cap: 0, len: 0, yaf: ()
        }
    }
    ...
    pub fn top_mut(&mut self) -> &mut ParseState {
        ...
        self.stack.get_mut(last_index).unwrap()
...
impl Parser {
    pub unsafe fn render_error_string(&self, text: &[u8], mut verbose: bool) -> *mut libc::c_uchar {
        ...
        //     };
        //     end = if offset.wrapping_add(30 as libc::c_int as usize) > text.len() {
        //         text.len()
        //     } else {
        ...
        //     }
        //     while start < end {
        //         if *text.add(start) as libc::c_int != '\n' as i32
        //             && *text.add(start) as libc::c_int != '\r' as i32
        //         {
        //             text[i] = *text.add(start) as libc::c_char;
        //         } else {
        //             text[i] = ' ' as i32 as libc::c_char;
        //         }
                ...
                ParseState::ParseComplete => {
                    if self.flags & ParserOption::AllowMultipleValues as u32 != 0 {
                        *self.stateStack.top_mut() = ParseState::GotValue;
                        ...
                        tok = self.lexer.lex(text, offset, &mut buf, &mut bufLen);
                        if tok != Token::Eof {
                            *self.stateStack.top_mut() = ParseState::ParseError;
                    ...
                    match tok {
                        ...
                        Token::StringWithEscapes => {
                                ...
                                && ((*self.callbacks).yajl_string).is_some()
                            {
                                ...
                                yajl_string_decode(self.decode_buf, buf, bufLen);
                                if ((*self.callbacks).yajl_string)
                                    .expect("non-null function pointer")(
                                    self.ctx,
                                    self.decode_buf.data(),
                                    self.decode_buf.len(),
                                ) == 0
                                {
                                    *self.stateStack.top_mut() = ParseState::ParseError;
                        ...
                        Token::Double => {
                            if !(self.callbacks).is_null() {
                                if ((*self.callbacks).yajl_number).is_some() {
                                    ...
                                    self.decode_buf.clear();
                                    self.decode_buf.append(buf as *const c_void, bufLen);
                                    buf = self.decode_buf.data();
                                    let (d, d_is_error) =
