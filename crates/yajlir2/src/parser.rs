...
impl Parser {
    pub fn new(options: ParserOptions) -> Self {
        ...
        // let afs = &mut afsBuffer;
        Self {
            callbacks: ptr::null(),
            ctx: ptr::null_mut(),
            // lexer: ptr::null_mut(),
            lexer: Lexer::new(options.allow_comments, !options.dont_validate_strings),
            parseError: None,
            bytesConsumed: 0,
            // decodeBuf: Buffer::alloc(),
            decode_buf: Vec::new(),
            stateStack: ByteStack::new2(),
            // alloc: (),
            flags: 0,
            options,
        }
...
impl Parser {
    ...
    pub unsafe fn parse(&mut self, text: &[u8]) -> Status {
        // self.ensure_lexer();
        self.do_parse(text)
    }
    ...
    pub unsafe fn get_error(
        ...
        self.render_error_string(text, verbose)
    }
