...
use super::{rlexer::Token, RLexer, ParseError, RParser};
        ...
        match self.state_stack.top() {
            ...
            _ => {
                if self.flags & ParserOption::AllowPartialValues as u32 == 0 {
                    *self.state_stack.top_mut() = ParseState::ParseError;
            ...
            match self.state_stack.top() {
                ParseState::ParseComplete => {
                    if self.flags & ParserOption::AllowMultipleValues as u32 != 0 {
                        *self.state_stack.top_mut() = ParseState::GotValue;
                    } else {
                        if self.flags & ParserOption::AllowTrailingGarbage as u32 != 0 {
                            break;
                        }
                        ...
                        tok =
                            (*self.lexer).lex(json_text,  offset, &mut buf, &mut bufLen);
                        if tok != Token::Eof {
                            *self.state_stack.top_mut() = ParseState::ParseError;
                ...
                | ParseState::ArrayStart => {
                    ...
                    match tok {
                        ...
                        Token::Error => {
                            *self.state_stack.top_mut() = ParseState::LexicalError;
                            continue;
                        }
                        Token::String => {
                            ...
                            if !(self.callbacks).is_null() {
                                if ((*self.callbacks).yajl_number).is_some() {
                                    ...
                                    let Ok(i) = parse_integer(buf, bufLen) else {
                                        *self.state_stack.top_mut() = ParseState::ParseError;
                                        self.parse_error = Some(ParseError::IntegerOverflow);
                                        if *offset >= bufLen {
                        ...
                        Token::Double => {
                            if !(self.callbacks).is_null() {
                                if ((*self.callbacks).yajl_number).is_some() {
                                    ...
                                    if d_is_error {
                                        *self.state_stack.top_mut() = ParseState::ParseError;
                                        self.parse_error = Some(ParseError::FloatingPointOverflow);
                                        if *offset >= bufLen {
                                            *offset = { *offset }.wrapping_sub(bufLen);
                                        } else {
                        ...
                        Token::RightBrace => {
                            if self.state_stack.top() == ParseState::ArrayStart {
                                if !(self.callbacks).is_null()
                        ...
                        _ => {
                            *self.state_stack.top_mut() = ParseState::ParseError;
                            self.parse_error = Some(ParseError::InvalidToken);
                            continue;
                        }
                    }
                    match current_block {
                        6407515180622463684 => {
                            let mut s = self.state_stack.top();
                            if s == ParseState::Start || s == ParseState::GotValue {
                                *self.state_stack.top_mut() = ParseState::ParseComplete;
                            } else if s == ParseState::MapNeedVal {
                                *self.state_stack.top_mut() = ParseState::MapGotVal;
                            } else {
                                *self.state_stack.top_mut() = ParseState::ArrayGotVal;
                            }
                            if stateToPush != ParseState::Start {
                                self.state_stack.push(stateToPush);
                            }
                        }
                        _ => {
                            *self.state_stack.top_mut() = ParseState::ParseError;
                ...
                ParseState::MapStart | ParseState::MapNeedKey => {
                    ...
                    match tok {
                        ...
                        Token::RightBracket => {
                            if self.state_stack.top() == ParseState::MapStart {
                                if !(self.callbacks).is_null()
                    ...
                    match current_block {
                        5544887021832600539 => {
                            ...
                            *self.state_stack.top_mut() = ParseState::MapSep;
                        }
                        _ => {
                            *self.state_stack.top_mut() = ParseState::ParseError;
                            self.parse_error = Some(ParseError::InvalidObjectKey);
                        }
                    }
                }
                ParseState::MapSep => {
                    ...
                    match tok {
                        Token::Colon => {
                            *self.state_stack.top_mut() = ParseState::MapNeedVal;
                        }
                        Token::Eof => return Status::Ok,
                        Token::Error => {
                            *self.state_stack.top_mut() = ParseState::LexicalError;
                        ...
                        _ => {
                            *self.state_stack.top_mut() = ParseState::ParseError;
                            self.parse_error = Some(ParseError::InvalidKeyValueSeparator);
                        }
                    }
                }
                ParseState::MapGotVal => {
                    ...
                    match tok {
                        ...
                        Token::Comma => {
                            *self.state_stack.top_mut() = ParseState::MapNeedKey;
                        }
                        Token::Eof => return Status::Ok,
                        Token::Error => {
                            *self.state_stack.top_mut() = ParseState::LexicalError;
                        ...
                        _ => {
                            *self.state_stack.top_mut() = ParseState::ParseError;
                            self.parse_error = Some(ParseError::InvalidObjectSeparator);
                            if *offset >= bufLen {
                                *offset = { *offset }.wrapping_sub(bufLen);
                            } else {
                                *offset = 0 as libc::c_int as usize;
                    ...
                    match tok {
                        Token::RightBrace => {
                                ...
                                ) == 0
                            {
                                *self.state_stack.top_mut() = ParseState::ParseError;
                                self.parse_error = Some(ParseError::ClientCancelled);
                                return Err("Status::ClientCanceled".into());
                                // return Status::ClientCanceled;
                            }
                            self.state_stack.pop();
                        }
                        Token::Comma => {
                            *self.state_stack.top_mut() = ParseState::ArrayNeedVal;
                        }
                        Token::Eof => return Ok(()),
                        // Token::Eof => return Status::Ok,
                        Token::Error => {
                        ...
                        _ => {
                            *self.state_stack.top_mut() = ParseState::ParseError;
                            self.parse_error = Some(ParseError::InvalidArraySeparator);
                        }
                    }
                }
            }
